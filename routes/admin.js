...
router.get("/products/:category/:filter", async (req, res) => {
    try {
        const userID = req.cookies.id
        const userDATA = await User.findOne({ _id: userID })
        const products = await Product.find({ colorID: null }).sort({ Date: -1 })
        if (userDATA.isAdmin == true) {
...
router.post("/add-product", upload.single("image"), async (req, res) => {
    try {
        ...
        if (userDATA.isAdmin == true) {
            const { name, minDes, des, price, mainCategory, sideCategory, brand, color } = req.body
...
router.put("/edit-product/:id/add-color", upload.single("image"), async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        const random = Math.floor(1000 + Math.random() * 92342 * Math.random() * 482382 * Math.random())
        const product = await Product.findOne({ _id: req.params.id })
        if (userDATA.isAdmin == true) {
            await Product.updateOne({ _id: req.params.id }, {
                $push: { colors: { color: req.body.color, colorID: random, image: req.file.filename } }
            })
            const newProduct = [
                new Product({
                    name: product.name,
                    ...
                    sideCategory: product.sideCategory
                })
            ]
...
router.put("/edit-product/:id/add-size", async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        const random = Math.floor(1000 + Math.random() * 92342 * Math.random() * 482382 * Math.random())
        ...
        if (userDATA.isAdmin == true) {
            await Product.updateOne({ _id: req.params.id }, {
                $push: { size: { size: req.body.size, sizeID: random, price: req.body.size_price } }
            })

            const newProduct = [
                new Product({
                    name: product.name + `(${req.body.size})`,
                    minDes: product.minDes,
                    des: product.des,
...
router.put("/edit-product/:id/dis", async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        const product = await Product.findOne({ _id: req.params.id })
        ...
        if (userDATA.isAdmin == true) {
            ...
            var x = Number(req.body.discount)
            var n = y - x;

            var persantage = Math.trunc((n / y) * 100)
            await Product.updateOne({ _id: req.params.id }, {
                $set: {
            ...
            await Product.updateMany({ mainProduct: req.params.id }, {
                $set: { 
                    old_price: product.price,
                    price: req.body.discount,
                    discount: req.body.discount,
...
router.put("/edit-product/:id/remove-dis", async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        const product = await Product.findOne({ _id: req.params.id })
...
router.put("/edit-product/remove-size/:id/:sizeID", async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        ...
        if(userDATA.isAdmin == true){
            await Product.updateOne({ _id: req.params.id }, {
                $pull: { size: { sizeID: Number(req.params.sizeID) } }
            })
...
router.put("/remove-color/:id/:colorID", async (req, res) => {
    try {
        const uesrID = req.cookies.id
        const userDATA = await User.findOne({ _id: uesrID })
        ...
        if(userDATA.isAdmin == true){
            await Product.updateOne({ _id: req.params.id }, {
                $pull: {colors: { colorID: Number(req.params.colorID) } }
            })
